...
async function sendEmail(to, subject, html, text) {
  const payload = JSON.stringify({ from: FROM, to: [to], subject, html, text });
...
exports.handler = async function(event) {
  ...
  try {
    ...
    if (type === 'reset') {
      // Password reset notification to Brian
      await sendEmail(
        BRIAN_EMAIL,
      ...
      // New registration notification to Brian
      await sendEmail(
        BRIAN_EMAIL,
      ...
      await sendEmail(
        ...
        'New Civitas Request',
        null,
        'New account request from ' + name + ' (' + email + '). Approve at civitasca.com/account.html'
      );
    }
